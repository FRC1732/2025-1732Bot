...
namespace NetworkTables
{
    public class Nt4Client
    {
        ...
        private readonly int[] _reconnectDelays = { 100, 200, 500, 1000, 2000, 3000, 5000, 8000, 13000, 21000 }; // Fibonacci-like backoff
        ...
        private (bool success, string errorMessage) AttemptConnection()
        {
            try
            {
                ...
                Debug.Log($"[NT4] Connecting to {_serverAddress} (attempt {_reconnectAttempts + 1}/{_maxReconnectAttempts})");
                _ws.Connect();
            ...
            {
                Debug.LogError($"[NT4] Connection attempt failed: {ex.Message}");
                ScheduleReconnect();
                return (false, ex.Message);
            }
        }
        ...
        private void OnOpen(object sender, EventArgs e)
        {
            ...
            // Restore subscriptions and publications
            RestoreState();
        ...
        {
            _lastMessageTime = DateTime.Now; // Update last message time for heartbeat
            if (args.Data != null)
            {
        ...
        private void OnClose(object sender, CloseEventArgs e)
        {
            ...
            if (_reconnectEnabled && !_reconnectInProgress)
            {
                ScheduleReconnect();
            }
        }
        ...
        {
            if (!_reconnectEnabled || _reconnectInProgress || _reconnectAttempts >= _maxReconnectAttempts)
            {
                return;
            }
            
            _reconnectInProgress = true;
            int delay = _reconnectAttempts < _reconnectDelays.Length ? 
                _reconnectDelays[_reconnectAttempts] : _reconnectDelays[_reconnectDelays.Length - 1];
            
            Debug.Log($"[NT4] Scheduling reconnect in {delay}ms (attempt {_reconnectAttempts + 1})");
            
            StopReconnectTimer();
            _reconnectTimer = new System.Threading.Timer(
                _ => UnityMainThreadDispatcher.Instance().Enqueue(() => ReconnectTimerCallback()), 
        ...
        {
            Debug.Log("[NT4] Attempting to reconnect...");
            AttemptConnection();
        }
        ...
        private void StartHeartbeatMonitor()
        {
            ...
            _heartbeatTimer = new System.Threading.Timer(
                _ => UnityMainThreadDispatcher.Instance().Enqueue(() => CheckHeartbeat()),
        ...
        private void CheckHeartbeat()
        {
            ...
            TimeSpan elapsed = DateTime.Now - _lastMessageTime;
            if (elapsed.TotalMilliseconds > _heartbeatTimeoutMs)
            {
                Debug.LogWarning($"[NT4] Heartbeat timeout: No messages received for {elapsed.TotalSeconds:F1}s");
                ...
                try
                {
                    if (_ws != null)
                    {
                        _ws.Close(CloseStatusCode.Away, "Heartbeat timeout");
                    }
                }
                catch (Exception ex)
                {
                    Debug.LogError($"[NT4] Error closing connection: {ex.Message}");
                }
                
                ScheduleReconnect();
            }
            else if (elapsed.TotalMilliseconds > _heartbeatIntervalMs)
            {
                // Send a timestamp message as a heartbeat
                WsSendTimestamp();
            }
        }
        ...
        private void RestoreState()
        {
            ...
            foreach (var topic in _publishedTopics.Values)
            {
